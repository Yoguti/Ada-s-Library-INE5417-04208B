    ...
    def apply(self, owner, target=None):
        if target and len(target) >= 2:
            ...
            if (0 <= index1 < len(display) and 0 <= index2 < len(display) and 
                index1 != index2):
                # Simple swap - no intermediate color changes
        ...
        if target and len(target) >= 2:
            ...
            if 0 <= book_index < len(display):
                ...
                if new_index != book_index:
                    # Remove book and insert at new position
...
class SwapWithOpponent(ActionCard):
    ...
    def apply(self, owner, target=None):
        ...
        if target and len(target) >= 3:
            owner_index, opponent_visual_index, opponent = target[0], target[1], target[2]
            owner_display = owner.get_display().get_display()
            opponent_display = opponent.get_display().get_display()
            
            # Convert visual opponent index to actual array index (mirrored)
            # If opponent has 10 books (0-9), visual index 0 = array index 9, visual index 9 = array index 0
